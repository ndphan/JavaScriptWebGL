...
class Font implements EngineEntity {
  ...
  calcTotalTextWidth(fontRef: FontReference) {
    ...
    const fontSize = fontRef.fontSize / this.pixelDensity;
    let carryWidth = 0;
  ...
  calculateFontSize(width: number, length: number): number {
    ...
    const widthFactor = 3;
    return ((width / fontRatio) * this.pixelDensity) / length / widthFactor;
  }
  ...
  createFonts(fontRef: FontReference): RenderFont[] {
    ...
    for (let charIdx = 0; charIdx < text.length; charIdx++) {
      ...
      const fontPos = new Rect2d(
        pos.x + carryWidth - fontChar.xOffset / 2.0,
    ...
    fontRef.displayRect.x = pos.x + carryWidth / 2.0;
    fontRef.displayRect.y = pos.y + maxHeight / 2.0;
